...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		int nonConsecutiveWordsCounter = 0; // Tracks sentences that don't end with punctuations.

		for (String token : tokens){
			if (isWord(token))
			{
				numWords++;
				nonConsecutiveWordsCounter++;
			...
			{
				nonConsecutiveWordsCounter = 0;
				numSentences++;
			}
		}
		if (nonConsecutiveWordsCounter >=2)
			numSentences++;
		//System.out.println("nonConsecutiveWordsCounter =="+nonConsecutiveWordsCounter);
	...
	public double getFleschScore()
	{
		...
		//System.out.println(" My text =="+this.text);
		double wordsSentenceRatio = Double.parseDouble(Integer.toString(this.getNumWords()))/Double.parseDouble(Integer.toString(this.getNumSentences()));
		double syllabuleWordsRatio = Double.parseDouble(Integer.toString(this.getNumSyllables()))/Double.parseDouble(Integer.toString(this.getNumWords()));
		double leftMultiply = 1.015 * wordsSentenceRatio;
		double rightMultiply = 84.6 * syllabuleWordsRatio;
		double differenceAddition = leftMultiply + rightMultiply;
		double fleshScore = 206.835 -differenceAddition;
		//System.out.println("Flesh Score=="+fleshScore+" wordsSentenceRatio =="+wordsSentenceRatio+" syllableWordsRatio=="+syllabuleWordsRatio+" leftMultiply=="+leftMultiply+" rightMultiply=="+rightMultiply+"differenceAddition=="+differenceAddition);
		return fleshScore;
	}
	...
	public static void main(String[] args)
	{
		...
		//testCase(new EfficientDocument("There is a change. It continues and asks how? And??"), 2, 10, 3);
		testCase(new EfficientDocument("Sentence"), 2, 1, 1);
		testCase(new EfficientDocument("Sentences?!"), 3, 1, 1);
		testCase(new EfficientDocument("What? Very Great! That cannot be you know"), 10, 8, 3);
